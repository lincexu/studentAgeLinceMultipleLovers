...
| 通用设置 | 锁定loverId | 关 | 开启后loverId不再自动切换（持久配置，区别于控制台LOCK的运行时锁定） |
...
| 格式 | 说明 |
|------|------|
| `5207, 1, X` | 恋人总数 ≥ X 时满足条件 |
| `5207, -1, X` | 恋人总数 ≤ X 时满足条件 |
| `5207, 2, 1, id1, id2, …` | 所有指定角色均为恋人 |
| `5207, 2, -1, id1, id2, …` | 所有指定角色均不是恋人 |
| `5207, 3, 1, X, Y` | 角色Y是第X个恋人（按顺序，1-based） |
...
详细兼容性说明和适配本模组请查看 [COMPATIBILITY.md](COMPATIBILITY.md)

## 九、构建方法

> 由于本mod都是打包好dll，可能会被软件视为病毒，不放心请自行构建打包
...
### v0.1.4 (2026-03-06)
- 新增自定义条件5207：恋人数量判断（`5207,1,X` / `5207,-1,X`）、批量身份检查（`5207,2,1,id...` / `5207,2,-1,id...`）、恋人顺序/位置判断（`5207,3,1,X,Y` / `5207,3,2,Y` / `5207,3,3,X,Y`）、至少一个匹配（`5207,14,1,id...` / `5207,14,-1,id...`）
- 新增自定义效果5217：全部恋人好感（`5217,0,X`）、排除好感（`5217,1,X`）、分手（`5217,6,1,X`）、恋人融洽度设置（`5217,7,X,Y`）
- 自定义条件显示已与官方同步：满足时绿色、不满足时红色，角色ID替换为角色名称
- 新增「锁定loverId」配置项：持久化锁定，区别于控制台LOCK的运行时锁定
